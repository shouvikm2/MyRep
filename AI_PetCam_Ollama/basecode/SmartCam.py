...
def ai_worker(ollama_url: str, model: str, interval: int, motion_threshold: int,
    ...
    while not stop_event.is_set():
        ...
        if frame_to_process is not None:
            current_frame_gray = cv2.cvtColor(frame_to_process, cv2.COLOR_BGR2GRAY)
...
def main(video_source: str | int, ollama_url: str, model: str, initial_prompt: str,
    ...
    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8)) if enhance_video else None
